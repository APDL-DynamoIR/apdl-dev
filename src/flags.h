...
template<int i>
struct SY_Flags_type
{
 typedef size_t type;
};

template<>
struct SY_Flags_type<1>
{
 typedef unsigned char type;
};

template<>
struct SY_Flags_type<2>
{
 typedef unsigned short type;
};

template<>
struct SY_Flags_type<5>
{
 typedef qint64 type;
};

template<>
struct SY_Flags_type<6>
{
 typedef qint64 type;
};

template<>
struct SY_Flags_type<7>
{
 typedef qint64 type;
};

template<>
struct SY_Flags_type<8>
{
 typedef qint64 type;
};
...
struct SY_Flag_Test
{
 ...
 unsigned long mask;
 SY_Flag_Test(Test_Type tt, void* v, size_t l, unsigned long m):test_type(tt), bitset(v), bitset_byte_length(l), mask(m)
 {

 }

 #define TEST_IMPL_RESTORE_BITS(byte_count) \
  SY_Flags_type<byte_count>::type* bits =  \
   reinterpret_cast< SY_Flags_type<byte_count>::type* >(bitset); \
...
typedef SY_Flags_type<byte_count>::type Flags_type; \
union { SY_Flags_type<byte_count>::type Flags; struct \
{
#else
#define flags_(byte_count) \
...
#define mask_(name, byte_count) \
 union { SY_Flags_type<byte_count>::type name##_##mask; struct {
#endif
...
#define _flags_ \
 ...
 SY_Flag_Test flags_all(unsigned long mask) \
 { \
  return SY_Flag_Test(SY_Flag_Test::Test_All, &Flags, _Flags_byte_count_, mask); \
 } \
 SY_Flag_Test flags_any(unsigned long mask) \
 { \
  return SY_Flag_Test(SY_Flag_Test::Test_Any, &Flags, _Flags_byte_count_, mask); \
 } \
 SY_Flag_Test flags_none(unsigned long mask) \
 { \
  return SY_Flag_Test(SY_Flag_Test::Test_None, &Flags, _Flags_byte_count_, mask); \
 }
#endif
